...
#ifndef INCLUDE_PF_FINDER_MULTIGOALFINDER_HPP_
#define INCLUDE_PF_FINDER_MULTIGOALFINDER_HPP_
...
class MultiGoalFinder
 ...
 public:
  std::tuple<PathResult, std::shared_ptr<Path<TPos>>, U64> findPathImpl(
    ...
    auto specialEstimate = [&](const TPos &from, const TPos &to) {
      return goalExist ? TEstimateEval::eval(from, to) : 0;
    };
    ...
    // add initial state
    heap.update(from, {0, specialEstimate(from, to)});
      ...
      for (const typename BASE::Direction &dir : directions) {
        ...
        for (TPos &newOffset : newOffsets) {
          ...
          if (found_it != infoTable.end()) {
            ...
            CostT newHCost = specialEstimate(newPos, to);
  ...
  MultiGoalFinder() = default;
  explicit MultiGoalFinder(const FinderConfig &_config) : BASE(_config) {}
};

}  // namespace pathfinding

#endif  // INCLUDE_PF_FINDER_MULTIGOALFINDER_HPP_
